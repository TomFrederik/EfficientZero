...
# class CartPoleGameHistory:
class GameHistory: # for debugging    
    """
    ...
    def __init__(self, action_space, max_length=200, config=None):
        ...
        if hasattr(config, "obs_shape"): # Otherwise fails silently if config does not have obs shape
            self.zero_obs_shape = (config.obs_shape[-2], config.obs_shape[-1], config.image_channel)
        else:
            self.zero_obs_shape = (4,)
        self.child_visits = []
        self.root_values = []
